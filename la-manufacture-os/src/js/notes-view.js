// Notes View Module (SPA integration)
...
export async function initNotesView() {
  ...
  try {
    ...
    console.error('[NotesView] Init error:', error);
...
function updateProjectSelects() {
  const projectFilter = document.getElementById('notesProjectFilter');
  if (projectFilter) {
...
async function saveNote() {
  try {
    ...
    const selectedColor = document.querySelector('#noteModal .color-btn.active');
...
function handleFilterChip(filter) {
  ...
  // Update active state
  document.querySelectorAll('#view-notes .filter-chip').forEach(chip => {
...
function setupEventListeners() {
  ...
  document.querySelectorAll('#noteModal .color-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      document.querySelectorAll('#noteModal .color-btn').forEach(b => b.classList.remove('active'));
  ...
  if (quickAddProjectBtn) {
    quickAddProjectBtn.addEventListener('click', quickAddProject);
  }
  ...
  document.querySelectorAll('#shareModal .modal-close, #shareModal .btn-secondary').forEach(btn => {
    btn.addEventListener('click', closeShareModal);
  });
}

// Export for app.js to close modals on view change
export function closeNotesModals() {
  closeNoteModal();
  closeShareModal();
}
